...
class DataSourceStatus:
    """Represents the availability of a well-data source."""
...
def _diagnose_oracle_environment(
    ...
    """Return diagnostics that explain why Oracle connectivity may fail."""
    ...
    for key, label in (("user", "Username"), ("password", "Password"), ("dsn", "DSN")):
        value = details.get(key)
        diagnostics.append(
            DataSourceStatus(
                f"Oracle {label.lower()}",
                bool(value),
                f"{label} {'is configured' if value else 'is missing'}.",
    ...
    if config_dir:
        ...
        diagnostics.append(
            DataSourceStatus(
                "TNS_ADMIN directory",
                config_path.exists(),
                "TNS_ADMIN points to the configured Oracle network directory." if config_path.exists() else "Configured TNS_ADMIN directory does not exist.",
                str(config_path),
            )
        )
    else:
        diagnostics.append(
            DataSourceStatus(
                "TNS_ADMIN directory",
                False,
                "No TNS_ADMIN directory configured. Set TNS_ADMIN or provide ORACLE_TNS_ADMIN in secrets when using TNS aliases.",
            )
        )

    tns_file: Optional[Path] = None
    if config_dir:
        candidate = Path(config_dir) / "tnsnames.ora"
        if candidate.exists():
            tns_file = candidate
        diagnostics.append(
            DataSourceStatus(
                "tnsnames.ora",
                candidate.exists(),
                "Found tnsnames.ora file." if candidate.exists() else "tnsnames.ora not located inside the configured TNS directory.",
                str(candidate),
            )
        )
    ...
    if wallet_dir:
        ...
        diagnostics.append(
            DataSourceStatus(
                ...
                wallet_path.exists(),
                "Wallet directory is accessible." if wallet_path.exists() else "Configured wallet directory does not exist.",
                str(wallet_path),
            )
        )
    ...
    if lib_dir:
        ...
        diagnostics.append(
            DataSourceStatus(
                ...
                lib_path.exists(),
                "Instant Client directory is accessible." if lib_path.exists() else "Configured Instant Client directory does not exist.",
                str(lib_path),
            )
        )

    dsn_value = details.get("dsn")
    if dsn_value and tns_file and tns_file.exists():
        alias = dsn_value.strip()
        # Ignore easy connect strings which contain spaces, slashes or parentheses
        if not any(token in alias for token in ("/", "(", ")", ":")):
            try:
                contents = tns_file.read_text(encoding="utf-8", errors="ignore")
            except Exception as exc:  # pragma: no cover - filesystem issues
                diagnostics.append(
                    DataSourceStatus(
                        "TNS alias lookup",
                        False,
                        f"Unable to read tnsnames.ora: {exc}",
                        str(tns_file),
                    )
                )
            else:
                pattern = re.compile(rf"^\s*{re.escape(alias)}\s*=", re.IGNORECASE | re.MULTILINE)
                found = bool(pattern.search(contents))
                diagnostics.append(
                    DataSourceStatus(
                        "TNS alias lookup",
                        found,
                        "DSN alias located in tnsnames.ora." if found else "DSN alias not found in tnsnames.ora. Verify the entry name.",
                        alias,
                    )
                )
    ...
    if not cached.is_empty():
        diagnostics.append(
            DataSourceStatus(
                "Parquet cache",
                True,
                "Loaded well metadata from the parquet snapshot.",
                str(PROCESSED_PARQUET),
            )
        )
        return ensure_standard_identifiers(cached), "parquet cache", diagnostics
    ...
    if PROCESSED_PARQUET.exists():
        diagnostics.append(
            DataSourceStatus(
                "Parquet cache",
                False,
                "Parquet cache exists but returned no records. Delete the file to rebuild it or replace it with a valid snapshot.",
                str(PROCESSED_PARQUET),
            )
        )
    else:
            ...
            if not rds_snapshot.is_empty():
                diagnostics.append(
                    DataSourceStatus(
                        "Legacy RDS snapshot",
                        True,
                        "Loaded well metadata from the RDS snapshot.",
                        str(PROCESSED_RDS),
                    )
                )
                return ensure_standard_identifiers(rds_snapshot), "legacy RDS snapshot", diagnostics
            diagnostics.append(
                DataSourceStatus(
                    "Legacy RDS snapshot",
                    False,
                    "RDS snapshot was read but did not return any records.",
                    str(PROCESSED_RDS),
                )
            )
    else:
    ...
    with st.expander(
        ...
        if diagnostics:
            for status in diagnostics:
                ...
                st.write(f"{icon} **{status.source}**{location} — {status.details}")
        else:
